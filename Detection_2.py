...
from math import *
from CharCNN import train
from KNN_detection import KNN_detection
from Gradient_Shaping import Gradient_Shaping
...
def detection_one_at_a_time(Nham, Nspam, Train, Para_Ham, Para_Spam, Attacks):
	...
	# ind_large = np.where(Para_Ham[1]>0.3)[0]
	# for i in ind_large:
	# 	print(Para_Ham[0])
	# 	print(len(X_h))
	# 	l = ceil(X_h[i].size/2)
	...
	# 	print(X_h[-1].size)
	# 	Para_Ham[0] += 1
	# 	Para_Ham[1][i] = Para_Ham[1][i]/2
	# 	Para_Ham[1] = np.concatenate((Para_Ham[1], np.array([Para_Ham[1][i]])))
	# 	Para_Ham[2] = np.concatenate((Para_Ham[2], Para_Ham[2][i].reshape((1,D))))
	# 	Para_Ham[4] = np.concatenate((Para_Ham[4], Para_Ham[4][i].reshape((1,D))))
	# 	Para_Ham[2][i] = Train[X_h[i]].sum(axis=0) / Train[X_h[i]].sum()
	# 	Para_Ham[2][-1] = Train[X_h[-1]].sum(axis=0) / Train[X_h[-1]].sum()
	# ind_large = np.where(Para_Spam[1]>0.3)[0]
	# for i in ind_large:
	# 	print(Para_Spam[0])
	# 	print(len(X_s))
	# 	l = ceil(X_s[i].size/2)
	...
	# 	print(X_s[-1].size)
	# 	Para_Spam[0] += 1
	# 	Para_Spam[1][i] = Para_Spam[1][i]/2
	# 	Para_Spam[1] = np.concatenate((Para_Spam[1], np.array([Para_Spam[1][i]])))
	# 	Para_Spam[2] = np.concatenate((Para_Spam[2], Para_Spam[2][i].reshape((1,D))))
	# 	Para_Spam[4] = np.concatenate((Para_Spam[4], Para_Spam[4][i].reshape((1,D))))
	# 	Para_Spam[2][i] = Train[X_s[i]].sum(axis=0) / Train[X_s[i]].sum()
	# 	Para_Spam[2][-1] = Train[X_s[-1]].sum(axis=0) / Train[X_s[-1]].sum()
	...
	# jj = Para_Ham[1].argmax()
	# print(jj, Para_Ham[1][jj])
	# ham_in_1 = Train[X_h[jj]]
	# hh = comp_log_likelihood(ham_in_1, Para_Ham).argmax(axis=1)
	# # hh = hh-hh.min(axis=1).reshape((ham_in_1.shape[0],1))
	# # hh = hh/hh.sum(axis=1).reshape((ham_in_1.shape[0],1))
	# # print(hh.std(axis=1))
	# # print(hh)
	# print(ham_in_1.shape[0], np.where(hh!=jj)[0].size, np.where(hh==jj)[0].size)
	# # np.savetxt("outputs/2000_4000_Ham.csv", comp_log_likelihood(Train[ind_ham], Para_Ham), delimiter=",")
	# # np.savetxt("outputs/2000_4000_Spam.csv", comp_log_likelihood(Train[ind_spam], Para_Spam), delimiter=",")
	# return
...
def main(dataset, classifier, NAttackHam = 0, NAttackSpam = 0, Syn = False):
	...
	if NAttackHam == 0 and NAttackSpam == 0:  # pure
		...
		# emailsTrainPath = 'dataset/'+dataset+'/Attack_{}_{}.train'.format(NAttackHam, NAttackSpam)
		# emailsTestPath = 'dataset/'+dataset+'/Attack_{}_{}.test'.format(NAttackHam, NAttackSpam)
	else: # poisoned
		...
		# paraFilteredSpamPath = 'learnedPara/'+dataset+'/learnedParaFiltered{}_Attack_{}_{}/'.format(class1[dataset], NAttackHam, NAttackSpam)
		...
		# HamGTlabels = np.load(HamGTlabelsPath)
		SpamGTlabelsPath = 'dataset/'+dataset+'/{}GroundTruthLabels_Attack_{}_{}.npy'.format(class1[dataset], NAttackHam, NAttackSpam)
		# SpamGTlabels = np.load(SpamGTlabelsPath)
		# emailsTrainPath = 'dataset/'+dataset+'/Attack_{}_{}.train'.format(NAttackHam, NAttackSpam)
		# emailsTestPath = 'dataset/'+dataset+'/Attack_{}_{}.test'.format(NAttackHam, NAttackSpam)
		# CharCNNPoisonedTrainPath = 'TextClassificationModelsPytorch/data/Attack_{}_{}.train.poisoned'.format(NAttackHam, NAttackSpam)
		# CharCNNTrainPath = 'TextClassificationModelsPytorch/data/Attack_{}_{}.train'.format(NAttackHam, NAttackSpam)
		# CharCNNTestPath = 'TextClassificationModelsPytorch/data/Attack_{}_{}.test'.format(NAttackHam, NAttackSpam)
		
	# load training & test data
	TrainHam = sparse.load_npz(featuresHamPath)
	TrainSpam = sparse.load_npz(featuresSpamPath)
	# TrainHam = TrainHam[:CleanHam+1000]
	# TrainSpam = TrainSpam[:CleanSpam+2893]
	...
	# Train_attack = sparse.vstack([TrainHam[CleanHam:], TrainSpam[CleanSpam:]])
	# label_attack = np.concatenate((np.ones(TrainHam.shape[0]-CleanHam),np.zeros(TrainSpam.shape[0]-CleanSpam)))
	# print(Train_attack.shape)
	# classifiers(Train_attack, label_attack, Train_attack, label_attack, 'SVM')
	print("poisoned {} classifier".format(classifier))
	# poisoned_acc_SVM += classifiers(Train, labelsTrain, Test, labelsTest, 'SVM')
	# poisoned_acc_LR += classifiers(Train, labelsTrain, Test, labelsTest, 'LR')
	# poisoned_acc_CharCNN += train.main(CharCNNTrainPath, CharCNNTestPath)
	# LSTM_classification('Attack_{}_{}_train.csv'.format(NAttackHam, NAttackSpam), 'test.csv')
		
	# -------------------BIC-based defense old--------------------------------
	'''
	m = 1
	A = np.zeros(Train.shape[0])
	repeat = 1
	for p in range(repeat):
		(s, attack, acc, tpr, fpr) = detection(TrainHam.shape[0], TrainSpam.shape[0], Train, labelsTrain, Para_Ham, Para_Spam, Attacks, Test, labelsTest, classifier, NCleanHam+NCleanSpam,m)
		A += attack
	# A = np.around(A/5)
	# A = A.astype('bool')
	A = A > repeat/2
	print('#######################after 5 iterations:{}'.format(A.sum()))
	'''
	'''
	# -------------------BIC-based defense--------------------------------
	...
	# print('#################remove true positive samples#######################')
	# # ind_new = np.logical_or(np.logical_not(TrueAttack),np.logical_not(Attacks))
	# ind = np.logical_and(Attacks, np.logical_not(A))
	# avg_likelihood_in_spam = comp_log_likelihood(Train[ind], Para_Spam).max(axis=1).sum()/ind.sum()
	# avg_likelihood_in_ham = comp_log_likelihood(Train[ind], Para_Ham).max(axis=1).sum()/ind.sum()
	# print(ind.sum(), avg_likelihood_in_ham, avg_likelihood_in_spam)
	# ind_new = np.logical_not(TrueAttack)
	# print("poisoned {} classifier".format(classifier))
	# classifiers(Train[ind_new], labelsTrain[ind_new], Test, labelsTest, classifier)

	# print('#################false positive samples#######################')
	# # ind_new = np.logical_or(np.logical_not(Attacks), FalseAttack)
	# print("total: {}".format(FalseAttack.sum()))
	# print("ham: {}".format(np.logical_and(np.logical_not(s),FalseAttack).sum()))
	# print("spam: {}".format(np.logical_and(s,FalseAttack).sum()))
	# Train = Train[FalseAttack]
	# labelsTrain = labelsTrain[FalseAttack]
	# print("\"clean\" {} classifier".format(classifier))
	# classifiers(Train, labelsTrain, Test, labelsTest, classifier)

	# 5. re-esitimate w_s and w_h
	# likelihood_under_ham = comp_log_likelihood(Train, Para_Ham).max(axis=1)
	# likelihood_under_spam = comp_log_likelihood(Train, Para_Spam).max(axis=1)
	# w_s = np.array(likelihood_under_ham > likelihood_under_spam).reshape(N,)
	# w_h = np.array(likelihood_under_spam > likelihood_under_ham).reshape(N,)
	# tmp = np.logical_or(np.logical_and(s, w_s), np.logical_and(np.logical_not(s), w_h))
	# # tmp = T => attack, tmp = F => normal
	# Train_new = Train[np.logical_not(tmp),:]
	
	poisoned_acc_SVM = poisoned_acc_SVM/folder
	poisoned_acc_LR = poisoned_acc_LR/folder
	poisoned_acc_CharCNN = poisoned_acc_CharCNN/folder
	test_acc_SVM = test_acc_SVM/folder
	test_acc_LR = test_acc_LR/folder
	test_acc_CharCNN = test_acc_CharCNN/folder
	...
	print("avg acc of poisoned CharCNN {}, avg acc of retrained CharCNN {}".format(poisoned_acc_CharCNN, test_acc_CharCNN))
...
if __name__ == "__main__":
	...
	main('full', classifier = 'SVM', NAttackHam = 2000, NAttackSpam = 2000)
	# main('AmazonReview', classifier = 'LR', NAttackHam = 10000, NAttackSpam = 0)